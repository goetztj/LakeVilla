...
std::atomic<uint32_t> ycsbc::LHTransactionsBankingQueryDBLvl0::next_id(0);

ycsbc::LHTransactionsBankingQueryDBLvl0::LHTransactionsBankingQueryDBLvl0(
...
void ycsbc::LHTransactionsBankingQueryDBLvl0::Init() {
  ...
  uint32_t t_id = ycsbc::LHTransactionsBankingQueryDBLvl0::next_id.fetch_add(1);
...
void ycsbc::LHTransactionsBankingQueryDBLvl0::Close() {
...
// check balance
int ycsbc::LHTransactionsBankingQueryDBLvl0::Read(
    const std::string& table, const std::string& key,
    const std::vector<std::string>* fields,
    std::vector<ycsbc::DB::KVPair>& result) {
  std::string path = "";

  LHTransactions::TransactionManager* manager = this->manager0.get();
  auto account_key_def = this->next_actor;
  ...
  auto account_key = std::to_string(account_key_def);
...
// withdraw
int ycsbc::LHTransactionsBankingQueryDBLvl0::Scan(
  ...
  *changes_log -= changes;
  ...
  std::vector<ycsbc::DB::KVPair> change_pair;
  change_pair.push_back({"balance", std::to_string(changes)});

  auto prev_id = manager->register_update_pairs(
      update_pair, LHTransactions::UpdateOperations::SUB, change_pair);
...
int ycsbc::LHTransactionsBankingQueryDBLvl0::Update(
    const std::string& table, const std::string& key,
    std::vector<ycsbc::DB::KVPair>& values) {
  LHTransactions::TransactionManager* manager_sender = this->manager0.get();
  auto account_key_def = this->next_actor;
  auto other_key_def = 0;
  ...
  auto prev_id = manager_sender->register_update_pairs(
      update_pair, LHTransactions::UpdateOperations::SUB, op_pair2);
  ...
  auto prev_id2 = manager_sender->register_update_pairs(
      update_pair2, LHTransactions::UpdateOperations::ADD, op_pair);
  manager_sender->remove_file(0, account_key, &path, prev_id2);
  ...
  bool add_success2 =
      manager_sender->add_file(0, arrow_table2, account_key, prev_id2);
...
int ycsbc::LHTransactionsBankingQueryDBLvl0::Insert(
    const std::string& table, const std::string& key,
    std::vector<ycsbc::DB::KVPair>& values) {
  return 0;
}

int ycsbc::LHTransactionsBankingQueryDBLvl0::Delete(const std::string& table,
                                                    const std::string& key) {
  return 0;
}

std::shared_ptr<arrow::Table>
ycsbc::LHTransactionsBankingQueryDBLvl0::arrow_table_builder(
...
void ycsbc::LHTransactionsBankingQueryDBLvl0::print_stats() {
