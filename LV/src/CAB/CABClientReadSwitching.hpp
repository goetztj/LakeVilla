...
struct CABClientReadSwitching : CABClient {
  uint32_t num_clients;

  CABClientReadSwitching(uint32_t num_clients, std::string& base_path,
                         uint64_t tendant, StorageConnector::MinIOConfig config,
                         uint32_t id, preparedContent* content);
