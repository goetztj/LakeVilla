...
std::atomic<uint32_t> ycsbc::LHTransactionsBankingTableDBLvl2::next_id(0);

ycsbc::LHTransactionsBankingTableDBLvl2::LHTransactionsBankingTableDBLvl2(
...
void ycsbc::LHTransactionsBankingTableDBLvl2::Init() {
  ...
  uint32_t t_id = ycsbc::LHTransactionsBankingTableDBLvl2::next_id.fetch_add(1);

  std::vector<bool> config = {true, true, true};
  std::vector<bool> config_2 = {true, true, true};
  ...
  std::stringstream s;
  s << this->settings->pathToBankingTable0 << " 1" << std::endl
    << this->settings->pathToBankingTable1 << " 1" << std::endl;

  auto log = s.str();
  StorageConnector::MinIOConnector conn(&config2);
  conn.write(PathToGlobalLog, log.c_str(), log.size());
}

void ycsbc::LHTransactionsBankingTableDBLvl2::Close() {
...
// check balance
int ycsbc::LHTransactionsBankingTableDBLvl2::Read(
...
// withdraw
int ycsbc::LHTransactionsBankingTableDBLvl2::Scan(
...
int ycsbc::LHTransactionsBankingTableDBLvl2::Update(
    const std::string& table, const std::string& key,
  ...
  if (base_id == 0) {
    manager_sender->open_new_table(this->settings->pathToBankingTable1);
  } else {
    manager_sender->open_new_table(this->settings->pathToBankingTable0);
  }
  ...
  auto success2 = manager_sender->read_file(1, account_key, &account_fields3,
                                            tmp_result3, path);
  ...
  manager_sender->remove_file(1, account_key, &path, prev_id2);
  ...
  bool add_success2 =
      manager_sender->add_file(1, arrow_table2, account_key, prev_id2);
...
int ycsbc::LHTransactionsBankingTableDBLvl2::Insert(
    const std::string& table, const std::string& key,
    std::vector<ycsbc::DB::KVPair>& values) {
  return 0;
}

int ycsbc::LHTransactionsBankingTableDBLvl2::Delete(const std::string& table,
                                                    const std::string& key) {
  return 0;
}

std::shared_ptr<arrow::Table>
ycsbc::LHTransactionsBankingTableDBLvl2::arrow_table_builder(
...
void ycsbc::LHTransactionsBankingTableDBLvl2::print_stats() {
